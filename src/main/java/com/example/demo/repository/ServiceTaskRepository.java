...
public interface ServiceTaskRepository extends JpaRepository<ServiceTimestamp, Long> {
    @Query(value = "SELECT new com.example.demo.utils.FrequentService(t.userID, t.count, s.id , s.name) from ServiceTimestamp t " +
            "INNER JOIN Service s ON s.id = t.serviceID ORDER BY t.count DESC,t.timestamp Desc"
    )
